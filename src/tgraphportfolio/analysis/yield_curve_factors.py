...
def ns_basis(maturities: np.ndarray, decay: float) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
...
def fit_pca(
    ...
    """
    # Standardize
    yields_std = (yields_matrix - yields_matrix.mean(axis=0)) / yields_matrix.std(axis=0)
...
def compute_interlayer_weights(
    ns_factors_by_issuer: dict[str, list[NSFactors]],  # issuer -> [factors_t1, factors_t2, ...]
    ...
    for issuer, factors_list in ns_factors_by_issuer.items():
        ...
        # 2. Normalize
        weight = (factor_loading_weight * factor_effect / (1 + factor_effect) +
