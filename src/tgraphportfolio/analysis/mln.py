...
One layer per distinct value of a user-chosen column. Nodes are the usual
``name_column``; every layer's network is built with the *same* measure, date
range, transforms, filter and threshold as the single network, so the multiplex
is directly comparable to the "Network" tab rather than a separate analysis.
...
def prepare_mln_frame(
    ...
    *,
    ...
    if status is not None and where_clause:
        status(f"MLN: loading with SQL filter: WHERE {where_clause}")
    df = load_table(cfg.db_path, cfg.table, columns=cols, where_clause=where_clause)
...
def layer_values_of(df: pl.DataFrame, layer_column: str) -> list[str]:
    """Distinct layer values, as strings, in sorted order."""
